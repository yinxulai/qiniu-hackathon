import { responseSchema } from '@server/helpers/schema'
import { routerSchema } from '@taicode/common-server'
import { z } from 'zod'

// ==================== 窗口类型定义 ====================

// 窗口类型枚举
export const WindowType = z.enum(['panel', 'debug', 'setting'])
export type WindowType = z.infer<typeof WindowType>

// 窗口信息（简化版）
export const WindowInfo = z.object({
  type: WindowType,
  isVisible: z.boolean(),
})
export type WindowInfo = z.infer<typeof WindowInfo>

// ==================== API Schema 定义 ====================

const showWindowDescription = `
显示 Siwei 主面板窗口

**功能说明：**
- 显示并聚焦 Siwei 主面板窗口
- 窗口固定在屏幕右侧，宽度 320px
- 如果窗口未创建，则自动创建新窗口
- 支持快捷键 Ctrl+Shift+V 快速唤醒

**使用场景：**
- 用户需要打开 Siwei 界面进行交互
- 通过 API 或快捷键激活 Siwei 助手功能
`

export const ShowWindowSchema = routerSchema({
  operationId: 'showWindow',
  summary: '显示 Siwei 主窗口',
  tags: ['窗口管理'],
  description: showWindowDescription,
  response: responseSchema(z.boolean()),
})

const hideWindowDescription = `
隐藏 Siwei 主面板窗口

**功能说明：**
- 隐藏 Siwei 主面板窗口但不关闭进程
- 窗口状态和数据保持在内存中
- 可通过快捷键 Ctrl+Shift+V 或 API 重新显示
- 隐藏后 Siwei 助手仍在后台运行，可处理语音唤醒

**使用场景：**
- 用户完成交互后暂时隐藏 Siwei 界面
- 保持 Siwei 助手后台运行状态
`

export const HideWindowSchema = routerSchema({
  operationId: 'hideWindow',
  summary: '隐藏 Siwei 主窗口',
  tags: ['窗口管理'],
  description: hideWindowDescription,
  response: responseSchema(z.boolean()),
})

const toggleWindowDescription = `
切换 Siwei 主面板窗口显示/隐藏状态

**功能说明：**
- 智能切换 Siwei 窗口可见性状态
- 当窗口可见时自动隐藏
- 当窗口隐藏时自动显示并聚焦
- 相当于快捷键 Ctrl+Shift+V 的 API 实现

**使用场景：**
- 快速切换 Siwei 助手界面显示状态
- 一键式 Siwei 窗口管理操作
`

export const ToggleWindowSchema = routerSchema({
  operationId: 'toggleWindow',
  summary: '切换 Siwei 窗口显示状态',
  tags: ['窗口管理'],
  description: toggleWindowDescription,
  response: responseSchema(z.boolean()),
})

const reloadWindowDescription = `
重新加载 Siwei 主面板窗口

**功能说明：**
- 强制重新加载 Siwei 主窗口的 React 界面内容
- 清除当前界面状态并重新初始化
- 保持窗口位置和大小不变

**使用场景：**
- 开发调试时刷新 Siwei 前端代码变更
- Siwei 界面出现异常时恢复正常状态
- 强制更新 Siwei 界面数据和状态
`

export const ReloadWindowSchema = routerSchema({
  operationId: 'reloadWindow',
  summary: '重新加载 Siwei 窗口',
  tags: ['窗口管理'],
  description: reloadWindowDescription,
  response: responseSchema(z.boolean()),
})

const quitAppDescription = `
完全退出 Siwei 应用

**功能说明：**
- 关闭所有 Siwei 窗口（主面板、调试、设置窗口）
- 终止所有 Siwei 后台服务和 MCP 连接
- 清理内存资源和临时文件
- 注销全局快捷键绑定

**使用场景：**
- 完全关闭 Siwei 应用
- 系统关机前的安全退出
- 重启 Siwei 应用前的清理操作
`

export const QuitAppSchema = routerSchema({
  operationId: 'quitApp',
  summary: '退出 Siwei 应用',
  tags: ['窗口管理'],
  description: quitAppDescription,
  response: responseSchema(z.boolean()),
})

// ==================== 通用窗口操作 Schema ====================

const openWindowDescription = `
打开指定类型的 Siwei 窗口

**功能说明：**
- 根据窗口类型打开对应的 Siwei 功能窗口
- 如果窗口不存在则自动创建并初始化
- 如果窗口已存在则显示并聚焦到前台
- 每种窗口类型采用不同的布局和定位策略

**支持的 Siwei 窗口类型：**
- **panel**: Siwei 主面板窗口 - 屏幕右侧固定，320px 宽度，用于语音交互
- **debug**: Siwei 调试窗口 - 屏幕居中显示，1200x800px，用于开发调试和日志查看
- **setting**: Siwei 设置窗口 - 屏幕居中显示，800x600px，用于配置管理

**使用场景：**
- 根据功能需求打开对应的 Siwei 专用窗口
- Siwei 多窗口工作流管理
`

export const OpenWindowSchema = routerSchema({
  operationId: 'openWindow',
  summary: '打开指定 Siwei 窗口',
  tags: ['窗口管理'],
  description: openWindowDescription,
  body: z.object({
    type: WindowType.describe('窗口类型'),
  }),
  response: responseSchema(z.boolean()),
})

const closeWindowDescription = `
关闭指定类型的 Siwei 窗口

**功能说明：**
- 安全关闭指定类型的 Siwei 窗口实例
- 保存窗口状态和用户数据（如适用）
- 释放窗口相关的内存资源
- 如果窗口不存在或已关闭则返回 false

**注意事项：**
- 关闭 Siwei 主面板窗口不会退出应用，仅隐藏界面
- Siwei 调试和设置窗口关闭后会完全销毁实例
- 可通过 openWindow 重新创建已关闭的 Siwei 窗口

**使用场景：**
- 完成特定功能后关闭对应 Siwei 窗口
- Siwei 内存优化和窗口管理
`

export const CloseWindowSchema = routerSchema({
  operationId: 'closeWindow',
  summary: '关闭指定 Siwei 窗口',
  tags: ['窗口管理'],
  description: closeWindowDescription,
  body: z.object({
    type: WindowType.describe('窗口类型'),
  }),
  response: responseSchema(z.boolean()),
})

// ==================== 语音交互 Schema ====================

const activateVoiceInputDescription = `
激活 Siwei 语音输入功能

**功能说明：**
- 自动显示 Siwei 主面板窗口并聚焦
- 向前端发送 Siwei 语音唤醒信号
- 启动 Siwei 语音输入模式，等待用户说话
- 相当于快捷键 Ctrl+Shift+Space 的 API 实现

**使用场景：**
- Siwei 语音唤醒检测到关键词后自动触发
- MCP 组件通过 API 调用激活 Siwei 语音功能
- 第三方集成通过 HTTP API 触发 Siwei 语音交互
`

export const ActivateVoiceInputSchema = routerSchema({
  operationId: 'activateVoiceInput',
  summary: '激活 Siwei 语音输入',
  tags: ['语音交互'],
  description: activateVoiceInputDescription,
  response: responseSchema(z.boolean()),
})

// ==================== DevTools 控制 Schema ====================

const devToolsDescription = `
控制指定 Siwei 窗口的开发者工具

**功能说明：**
- 支持打开、关闭、切换 Siwei 指定窗口的 DevTools
- 可用于调试和开发时的问题排查
- 仅在开发环境或调试需要时使用

**Siwei 窗口类型：**
- panel: Siwei 主面板窗口
- debug: Siwei 调试窗口  
- setting: Siwei 设置窗口
`

export const OpenDevToolsSchema = routerSchema({
  operationId: 'openDevTools',
  summary: '打开指定 Siwei 窗口的 DevTools',
  tags: ['开发工具'],
  description: devToolsDescription,
  request: {
    params: z.object({
      type: WindowType.describe('窗口类型'),
    }),
  },
  response: responseSchema(z.boolean()),
})

export const CloseDevToolsSchema = routerSchema({
  operationId: 'closeDevTools',
  summary: '关闭指定 Siwei 窗口的 DevTools',
  tags: ['开发工具'],
  description: devToolsDescription,
  request: {
    params: z.object({
      type: WindowType.describe('窗口类型'),
    }),
  },
  response: responseSchema(z.boolean()),
})

export const ToggleDevToolsSchema = routerSchema({
  operationId: 'toggleDevTools',
  summary: '切换指定 Siwei 窗口的 DevTools 开关状态',
  tags: ['开发工具'],
  description: devToolsDescription,
  request: {
    params: z.object({
      type: WindowType.describe('窗口类型'),
    }),
  },
  response: responseSchema(z.boolean()),
})
